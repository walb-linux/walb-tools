...
inline uint32_t getLogicalBlockSize(int fd)
{
    ...
    if (::ioctl(fd, BLKSSZGET, &lbs) < 0) {
        throwLibcError("Geting logical block size failed.");
    }
    assert(lbs > 0);
    return lbs;
}
...
inline uint32_t getPhysicalBlockSize(int fd)
{
    ...
    if (::ioctl(fd, BLKPBSZGET, &pbs) < 0) {
        throwLibcError("Getting physical block size failed.");
    }
    assert(pbs > 0);
    return pbs;
}
...
inline uint64_t getBlockDeviceSize(int fd)
{
    if (isBlockDevice(fd)) {
        ...
        if (::ioctl(fd, BLKGETSIZE64, &size) < 0) {
            throwLibcError("ioctl(BLKGETSIZE64) failed.");
        }
        return size;
    } else {
        struct stat s;
...
inline void flushBufferCache(int fd)
{
    if (::ioctl(fd, BLKFLSBUF, 0) < 0) {
        throwLibcError("ioctl(BLKFLSBUF) failed.");
    }
}
...
inline void issueDiscard(int fd, uint64_t offsetLb, uint64_t sizeLb)
{
    ...
    if (::ioctl(fd, BLKDISCARD, &range) < 0) {
        throwLibcError("ioctl(BLKDISCARD) failed.");
    }
}
