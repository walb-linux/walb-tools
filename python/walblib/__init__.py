...
def verify_type(obj, typeValue, elemType=None, allowNone=False):
    ...
    if not isinstance(typeValue, list):
        typeValue = [typeValue]
    def is_class_obj(t):
        return isinstance(t, types.TypeType) or isinstance(t, types.ClassType)
    if not all([is_class_obj(t) for t in typeValue]):
    ...
    if not isinstance(elemType, list):
        elemType = [elemType]
    if not all([is_class_obj(t) for t in elemType]):
...
def get_server_params(s, sLayout=None, isDebug=False, useRepeater=False, maxFgTasks=2, maxBgTasks=1, maxConn=None, timeoutS=None):
    '''
    ...
    timeoutS :: int or None - connection timeout [sec].
    ...
    verify_type(timeoutS, int, allowNone=True)
